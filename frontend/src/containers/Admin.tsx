...
import { useAddCandidateMutation, useListCandidatesForStateQuery } from '../api';
import React from 'react';

const Admin: React.FC = () => {
    const [state, setState] = React.useState<string>('AL');
    const token = useSelector((state) => state.auth.token);
    const { data, refetch } = useListCandidatesForStateQuery({state}, {
        skip: !token
    });
        ...
        <div id='admin'>
            <div className="box">
                <div className="field">
                    ...
                    <button className="button is-primary" onClick={addCandidate}>Add Candidate</button>
                </div>
            </div>
        </div>
    )
